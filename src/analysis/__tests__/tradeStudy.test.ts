...
describe('solveForTarget (spec inversion)', () => {
  it('finds the minimum camera resolution for a target P_negate (increasing)', () => {
    // Target 0.5: reachable via resolution alone. (With the gimbal-less
    ...
    const p = getParam('optics.h_resolution_px')!
    const res = solveForTarget(defaultScenario(), p, 0.5, { min: 320, max: 12000, steps: 201 })
    ...
    expect(below).toBeLessThan(0.5 + 1e-6)
    expect(above).toBeGreaterThanOrEqual(0.5 - 1e-6)
  })
